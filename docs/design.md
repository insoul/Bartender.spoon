...
2. 술어 P(w) = "구분자 왼쪽에 보이는 항목이 없다"(구분자 자신이 접혔으면 참)는 폭에 대해
   단조다. `lo=0, hi=주 화면 폭` 이분 탐색으로 P 가 처음 참이 되는 임계 폭 T 를 오차 1pt 로
   짚는다. 구분자가 왼쪽으로 `mid/2` 만큼도 움직이지 않았으면(폭이 배치에 반영되지 않는
   구간) 너무 넓은 것으로 보아 `hi = mid`. 8pt 미만은 움직임이 판독 오차 안이라 검사하지 않는다.
3. 폭을 T 로 두고 한 번 더 판독한다. 구분자가 보이고 왼쪽이 다 접혀 있으면 T 로 끝낸다.
   구분자까지 접혀 있으면 만족 구간이 없는 것이다 — 폭 0 으로 두고 `hs.printf` 로 남기며,
   이때의 서명을 함께 돌려주어 호출자가 보관한다. 다음 호출은 0 번 항목에서 이 서명을 본다.

   세 조건(이웃 접힘·구분자 보임·반영 여부)을 한 번에 보며 만족값을 더듬지 않는 이유:
   만족 구간은 T 부터 구분자 자신이 접히는 폭 직전까지인데, `«` 는 항목 경계 단위로만
   움직여서 왼쪽이 다 접힌 다음 경계가 구분자 오른쪽 끝이다. 메뉴바가 빡빡하면 이 구간이
   몇 pt 에 불과하다(실측 120 이상 125 미만). 이분 탐색이 그 안에 떨어지는 것은 운이다.
   T 를 정확히 짚으면 구간 폭과 무관하게 찾는다.
