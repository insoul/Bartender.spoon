...
- `«` 의 x 를 찾는다. 없으면 `chevronX = nil`.
- 버튼이 `»` 상태면 `expanded = true` 로 표시한다. 접힌 항목이 없다는 뜻이고,
  사용자가 `«` 를 눌러 펼쳐 둔 상태도 여기에 해당한다.
...
0. **먼저 지금 폭 그대로 판독해 상태를 점검한다.**
   - `expanded` 면 아무것도 바꾸지 않고 끝낸다. 사용자가 펼쳐 둔 것을 다시 접지 않는다.
