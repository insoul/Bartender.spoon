...
점검에서 **구분자 왼쪽에 보이는 항목이 없으면 폭을 건드리지 않는다.** 구분자 자신이 접혀
있어도 마찬가지다 — Xcode 처럼 메뉴가 넓은 앱이 앞에 오면 예산이 줄어 시스템이 왼쪽 항목 다음으로
구분자를 접는데, 접어 둔 항목은 그대로 접혀 있고 앱을 바꾸면 시스템이 배치를 되돌리므로 손댈 것이
없다(실측). 이때 왼쪽 항목은 구분자가 마지막으로 보였을 때 기억한 목록으로 가린다.
